...
#[contractimpl]
impl MerchantVault {
    /// Initialize the contract with multi-sig signer set and threshold.
    pub fn initialize(
        env: Env,
        admin: Address,
    ...
    pub fn credit(env: Env, merchant_id: Address, amount: i128) -> Result<i128, Error> {
        let payment_router: Address = env
            .storage().instance().get(&DataKey::PaymentRouter)
            .ok_or(Error::NotInitialized)?;
        ...
        let current: i128 = env.storage().persistent()
            .get(&DataKey::Balance(merchant_id.clone())).unwrap_or(0);
    ...
    pub fn debit(env: Env, merchant_id: Address, amount: i128) -> Result<i128, Error> {
        let payout_contract: Address = env
            .storage().instance().get(&DataKey::PayoutContract)
            .ok_or(Error::NotInitialized)?;
        ...
        let current: i128 = env.storage().persistent()
            .get(&DataKey::Balance(merchant_id.clone())).unwrap_or(0);
        if current < amount { return Err(Error::InsufficientBalance); }
    ...
    /// Propose a withdrawal. Caller must be a registered signer.
    /// The proposer's approval is pre-recorded. Returns the proposal_id.
    pub fn propose_withdrawal(
        ...
        let expiry_ledgers: u32 = env.storage().instance()
            .get(&symbol_short!("exp_ldgrs")).unwrap_or(17_280);
    ...
    /// Approve a pending proposal. Executes the withdrawal when threshold is reached.
    ...
    pub fn approve_withdrawal(
        ...
        if proposal.executed  { return Err(Error::ProposalAlreadyExecuted); }
        if env.ledger().sequence() > proposal.created_ledger + proposal.expiry_ledgers {
            return Err(Error::ProposalExpired);
        }
        for existing in proposal.approvals.iter() {
            if existing == approver { return Err(Error::AlreadyApproved); }
        }
        ...
        if executed {
            ...
            env.storage().persistent().set(&DataKey::Proposal(proposal_id), &proposal);
            let current: i128 = env.storage().persistent()
                .get(&DataKey::Balance(proposal.merchant_id.clone())).unwrap_or(0);
            if current < proposal.amount { return Err(Error::InsufficientBalance); }
    ...
    pub fn balance_of(env: Env, merchant_id: Address) -> Result<i128, Error> {
        ...
        Ok(env.storage().persistent().get(&DataKey::Balance(merchant_id)).unwrap_or(0))
    }
    ...
    pub fn is_signer(env: Env, address: Address) -> bool {
        let signers: Vec<Address> = env.storage().instance()
            .get(&DataKey::Signers).unwrap_or_else(|| vec![&env]);
    ...
    fn assert_signer(env: &Env, address: &Address) -> Result<(), Error> {
        let signers: Vec<Address> = env.storage().instance()
            .get(&DataKey::Signers).ok_or(Error::NotInitialized)?;
        if !signers.contains(address) { return Err(Error::NotASigner); }
        Ok(())
    }
}
